...
@pytest.mark.stable
class TestSwapEx_109:

    @allure.step('前置条件')
    def setup(self):
        print('测试步骤：'
              '\n*、下限价单；买入开仓（开多）'
              '\n*、用户下2笔不同价格的订单，数量为2，两笔订单分别成交数量1'
              '\n*、验证开多订单撮合成功（查询撮合表有数据）')
    ...
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            ...
            orderIdList = []
            for i in range(2):
                orderInfo = user01.swap_order(contract_code=contract_code, price=round(self.currentPrice *(1+ (i+1)*0.01), 2),
                                            direction='buy',volume=2)
                orderIdList.append(orderInfo['data']['order_id'])
                # 用于成交
                user02.swap_order(contract_code=contract_code, price=round(self.currentPrice *(1+ (i+1)*0.01), 2),
                                  direction='sell')
            for i in range(2):
