...
@pytest.mark.stable
class TestSwapEx_073:

    @allure.step('前置条件')
    def setup(self):
        print('测试步骤：'
              '\n*、下限价单；买入开仓（开多）,同一笔订单部分数量成交'
              '\n*、验证撮合成功（查询撮合表有数据）'
              '\n*、执行撤单')

    @allure.title('撮合 买入开仓部分成交 撤单')
    @allure.step('测试执行')
    @pytest.mark.flaky(reruns=3, reruns_delay=3)
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            self.currentPrice = ATP.get_current_price()  # 最新价
            user01.swap_order(contract_code=contract_code, price=round(self.currentPrice * 0.99, 2),
                                  direction='sell')
            orderInfo = user01.swap_order(contract_code=contract_code, price=round(self.currentPrice * 0.99, 2),
