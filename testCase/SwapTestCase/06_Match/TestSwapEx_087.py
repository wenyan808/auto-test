...
@pytest.mark.stable
class TestSwapEx_087:

    @allure.step('前置条件')
    @pytest.fixture(scope='function', autouse=True)
    def setup(self,contract_code):
        print('测试步骤：'
              '\n*、下限价单；买入平仓（平空）,同一笔订单部分成交'
              '\n*、验证撮合成功（查询撮合表有数据）')
    ...
    @allure.title('撮合 买入平仓部分成交单笔订单 ')
    @allure.step('测试执行')
    @pytest.mark.flaky(reruns=3, reruns_delay=3)
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            time.sleep(1)
            user01.swap_order(contract_code=contract_code, price=round(self.currentPrice * 0.5, 2),
                                  direction='sell', offset='close',volume=5)
            orderInfo = user01.swap_order(contract_code=contract_code, price=round(self.currentPrice * 0.5, 2),
