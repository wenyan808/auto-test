...
from common.SwapServiceAPI import SwapService
...
class TestCoinswapTriggerOrder_006:
    ...
    def test_execute(self, symbol, symbol_period):
        with allure.step('1、登录币本位永续界面'):
            ...
            tp_order_plan = self.current_user.swap_trigger_order(contract_code=self.contract_code, trigger_type="ge", trigger_price=self.trigger_price, order_price=self.order_price, volume=10, direction="sell", offset="close", lever_rate=5)
        ...
        with allure.step('7、查看当前委托列表中的计划委托有结果B'):
            ...
            for o in all_order_plans:
                if o.get("order_id") == self.tp_order_id:
                    expected_info = {"symbol": self.symbol, "trigger_type": "ge", "volume": 10, "order_type": 1, "direction": "sell", "offset": "close", "trigger_price": self.trigger_price, "order_price": self.order_price, "status": 2}
