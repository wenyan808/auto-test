...
class TestCoinswapTriggerOrder_008:
    ...
    def test_execute(self, symbol, symbol_period):
        with allure.step('1、登录币本位永续界面'):
            ...
            self.order_price = round(self.last_price * 1.1, 1)
            sl_order_plan = self.current_user.swap_trigger_order(contract_code=self.contract_code, trigger_type="le", trigger_price=self.trigger_price, order_price=self.order_price, volume=10, direction="buy", offset="close", lever_rate=5)
        ...
        with allure.step('6、点击止损按钮有结果A'):
            ...
            for o in all_order_plans:
                if o.get("order_id") == self.sl_order_id:
                    expected_info = {"symbol": self.symbol, "trigger_type": "le", "volume": 10, "order_type": 1, "direction": "buy", "offset": "close", "trigger_price": self.trigger_price, "order_price": self.order_price, "status": 2}
