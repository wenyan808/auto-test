...
步骤/文本
    1、登录币本位永续界面
    2、在当前持仓tab选择持仓BTC/USD/USDT-多单，点击计划委托按钮
    3、选择计划止损按钮
...
from config.conf import URL, LSS_ACCESS_KEY, LSS_SECRET_KEY, COMMON_ACCESS_KEY, COMMON_SECRET_KEY
...
class TestCoinswapTriggerOrder_009:
    ...
    @allure.title('计划止损最优5/10/20挡')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period):
        with allure.step('1、登录币本位永续界面'):

            self.trigger_price = round(self.last_price * 0.9, 1)
            self.order_price = self.trigger_price
            sl_order_plan = self.current_user.swap_trigger_order(contract_code=self.contract_code, trigger_type="le", trigger_price=self.trigger_price, order_price=self.order_price, volume=10, direction="buy", offset="close", lever_rate=5, order_price_type="optimal_5")
            assert sl_order_plan.get("status") == "ok", f"下计划止损单失败, {sl_order_plan}"
            self.sl_order_id = sl_order_plan.get("data").get('order_id')
        with allure.step('2、在当前持仓tab选择持仓BTC/USD/USDT-多单，点击计划委托按钮'):
            pass
        ...
        with allure.step('6、点击止损按钮有结果A'):
            ...
            for o in all_order_plans:
                if o.get("order_id") == self.sl_order_id:
                    expected_info = {"symbol": self.symbol, "trigger_type": "le", "volume": 10, "order_type": 1, "direction": "buy", "offset": "close", "trigger_price": self.trigger_price, "order_price": 0, "order_price_type": "optimal_5", "status": 2}
                    assert common.util.compare_dict(expected_info, o)
                    return
            raise BaseException("在当前委托单{all_order_plans}中未找到计划委托单{sl_order_id}".format(all_order_plans=all_order_plans, sl_order_id=self.sl_order_id))
    ...
    def teardown(self):
        print('撤单')
        r_cancel = self.current_user.swap_trigger_cancel(contract_code=self.contract_code, order_id=self.sl_order_id)
