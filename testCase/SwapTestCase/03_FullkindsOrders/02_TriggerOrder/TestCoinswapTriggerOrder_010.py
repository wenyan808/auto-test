...
@allure.story('触发计划委托订单开仓测试')  # 这里填子功能，没有的话就把本行注释掉
class TestCoinswapTriggerOrder_010:
    ...
    def test_execute(self, symbol, symbol_period):
        with allure.step('1、登录币本位永续界面'):
            r_plan_open = self.current_user.swap_trigger_order(contract_code=self.contract_code, trigger_type=self.trigger_type, trigger_price=self.trigger_price, order_price=self.order_price, order_price_type="limit", volume=10, direction="buy", offset="open",
                                                               lever_rate=self.lever_rate)
            assert r_plan_open.get("status") == "ok", f"开仓下计划单失败: {r_plan_open}"
            self.order_id = r_plan_open.get("data").get("order_id")
        ...
        with allure.step('5、输入买入量10张'):
            pass
        with allure.step('6、点击买入开多按钮有结果A'):
            pass
        with allure.step('7、查看当前委托列表中的计划委托有结果B'):
            ...
            for o in all_order_plans:
                if o.get("order_id") == self.order_id:
                    expected_info = {"symbol": self.symbol, "trigger_type": "ge", "volume": 10, "order_type": 1, "direction": "buy", "offset": "open", "trigger_price": self.trigger_price, "order_price": self.order_price, "status": 2}
        ...
        with allure.step('8、最新价达到触发价时，正常触发订单后，检查计划委托列表和限价委托列表有结果C'):
            ...
            expected_info = {"symbol": self.symbol, "contract_code": self.contract_code, "volume": 10, "order_type": 1, "direction": "buy", "offset": "open", "status": 3}
