...
class TestCoinswapLimitOrder_003:
    ids = ["TestCoinswapLimitOrder_003", "TestCoinswapLimitOrder_004"]
    params = [
        {
    ...
    def setup_class(cls):
        with allure.step("变量初始化"):
            ...
            pass
            with allure.step("挂盘"):
                user02.swap_order(contract_code=cls.contract_code,price=round(cls.latest_price*0.99,2),direction='buy')
                user02.swap_order(contract_code=cls.contract_code,price=round(cls.latest_price*1.01,2),direction='sell')
                pass
    ...
    def test_execute(self, params):
        ...
        with allure.step('操作：下限价单'):
            orderId = user01.swap_order(contract_code=self.contract_code,price=round(self.latest_price*params['ratio'],2),
