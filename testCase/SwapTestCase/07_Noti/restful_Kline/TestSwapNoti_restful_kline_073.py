...
@allure.story('restful请求K线1mon 选填参数验证：只传参from,to')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_restful_kline_073:
    ...
    @allure.title('restful请求K线1mon 传参from,to')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            period = '1mon'
            toTime = int(time.time())
            fromTime = toTime - 60 * 60 * 24 * 30 *12
