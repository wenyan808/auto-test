...
@allure.story('restful请求K线30min 选填参数验证：只传参size')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_restful_kline_029:
    ...
    @allure.title('restful请求K线30min 传参size')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            ...
            assert 'market.' + contract_code + '.kline.30min' in kLineInfo['ch']
            if 'data' in kLineInfo:
                ...
                while n< len(kLineInfo['data']):
                    ...
                    if n != len(kLineInfo['data']) - 1:
                        assert kLineInfo['data'][n]['id'] + 60*30 == kLineInfo['data'][n+1]['id']
                    n = n + 1
            else:
