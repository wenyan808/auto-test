...
@allure.story('restful请求K线1mon 选填参数验证：只传参size')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_restful_kline_074:
    ...
    @allure.title('restful请求K线1mon 传参size')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            ...
            assert 'market.' + contract_code + '.kline.1mon' in kLineInfo['ch']
            if 'data' in kLineInfo:
                ...
                while n< len(kLineInfo['data']):
                    ...
                    # 断言k线的连续  30天259200  31天2678400 28天2419200
                    if n != len(kLineInfo['data']) - 1:
                        assert int(kLineInfo['data'][n+1]['id'] - kLineInfo['data'][n]['id']) in [2419200,2678400,2592000]
                    n = n + 1
            else:
