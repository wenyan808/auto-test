...
# @Author : 
    用例标题
        restful请求K线4hour 不传from,to,size
    前置条件
...
@allure.story('restful请求K线4hour 选填参数验证：不传from,to,size')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_restful_kline_054:
    ...
    @allure.title('restful请求K线4hour 不传from,to,size')
    @allure.step('测试执行')
