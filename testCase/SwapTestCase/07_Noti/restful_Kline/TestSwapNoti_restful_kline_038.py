...
@allure.story('restful请求K线60min 选填参数验证：只传参size')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_restful_kline_038:

    @allure.step('前置条件')
    def setup(self):
        print("\n自动化步骤："
              "\n*、发送restful请求kline的请求,类型=60min；"
              "\n*、选填参数是否传参：from【N】 ,to【N】 , size【Y】"
              "\n*、验证Kline返回结果："
              "\n\t返回topic正确，data内所有字段校验，不存在空的情况"
              "\n\tdata内所有数据时间连续")

    @allure.title('restful请求K线60min 传参size')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            ...
            assert 'market.' + contract_code + '.kline.60min' in kLineInfo['ch']
            if 'data' in kLineInfo:
                ...
                while n< len(kLineInfo['data']):
                    ...
                    if n != len(kLineInfo['data']) - 1:
                        assert kLineInfo['data'][n]['id'] + 60*60 == kLineInfo['data'][n+1]['id']
                    n = n + 1
            else:
