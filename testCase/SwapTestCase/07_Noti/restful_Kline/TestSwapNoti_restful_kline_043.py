...
@allure.story('restful请求K线60min 选填参数验证：to传参错误 非时间戳')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_restful_kline_043:
    ...
    @allure.title('restful请求K线60min to传参错误 非时间戳')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            ...
            kLineInfo = swap_api.swap_kline(contract_code=contract_code, period=period, size=size, to='a',
                                            From=fromTime)
