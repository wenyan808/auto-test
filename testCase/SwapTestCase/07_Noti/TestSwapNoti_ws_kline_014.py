...
@allure.story('WS订阅K线(req 传参from,to) 60min')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_ws_kline_014:
    ...
    def setup(self, contract_code, lever_rate, offsetO, offsetC, directionB, directionS):
        print("\n自动化步骤："
              "\n*、先执行两笔成交，更新K线数据；"
              "\n*、再发送req请求from（now - 60分钟） to(now)kline 60min的请求；"
              "\n*、验证Kline 60min返回结果；各返回值不为空[]")
