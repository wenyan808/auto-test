...
@pytest.mark.stable
class TestSwapNoti_restful_trade_006:
    ids = ['TestSwapNoti_restful_trade_006']
    params = [{'title': 'TestSwapNoti_restful_trade_006', 'case_name': '批量获取市场最近成交记录-传合约'}]
    ...
    def test_execute(self, params):
        ...
        with allure.step('验证:返回各字段不为空'):
            cols = ['amount', 'quantity', 'price', 'direction']
            for data in r['data'][0]['data']:
                for col in cols:
