...
@allure.story('WS订阅K线(req 传参from,to) 30min')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_ws_kline_013:
    ...
    def setup(self, contract_code, lever_rate, offsetO, offsetC, directionB, directionS):
        print("\n自动化步骤："
              "\n*、先执行两笔成交，更新K线数据；"
              "\n*、再发送req请求from（now - 30分钟） to(now)kline 30min的请求；"
              "\n*、验证Kline 30min返回结果；各返回值不为空[]")
    ...
    @allure.title('WS订阅K线(req 传参from,to) 30min')
    @allure.step('测试执行')
    def test_execute(self):
        with allure.step('详见官方文档'):
            self.period = '30min'
            self.toTime = int(time.time())
            self.fromTime = self.toTime - 60 * 30
            subs = {
