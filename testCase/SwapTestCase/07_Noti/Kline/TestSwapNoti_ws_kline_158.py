...
@allure.story('WS订阅K线(req) period大小写')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_ws_kline_158:

    @allure.step('前置条件')
    def setup(self):
        print("\n自动化步骤："
              "\n*、发送req请求from to请求kline ，请求参数中period大小写；"
              "\n*、验证Kline 1min返回结果；正常返回，大小写不敏感")
    ...
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            ...
            assert dict(result).get('err-code') == None #如果有error即失败
            pass
