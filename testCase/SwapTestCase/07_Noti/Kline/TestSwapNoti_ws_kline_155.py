...
@allure.story('WS订阅K线(req) 1week数据超过2000条')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_ws_kline_155:

    @allure.step('前置条件')
    def setup(self):
        print("\n自动化步骤："
              "\n*、发送req请求from to请求kline ，1week数据超过2000条；"
              "\n*、验证Kline 1min返回结果：目前数据量只有1年，按week算不会超过2000；此处不验证超量情况，只验证数量是否拿到")

    @allure.title('WS订阅K线(req) 1week数据超过2000条')
    @allure.step('测试执行')
