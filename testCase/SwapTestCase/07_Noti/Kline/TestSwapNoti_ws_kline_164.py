...
@allure.story('WS订阅K线(req) to格式不合法')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_ws_kline_164:

    @allure.step('前置条件')
    def setup(self):
        print("\n自动化步骤："
              "\n*、发送req请求from to请求kline ，请求参数中to格式不合法，写入字符串或许非法字符；"
              "\n*、验证Kline 1min返回结果；报错 请求失败")

    @allure.title('WS订阅K线(req) to格式不合法')
    @allure.step('测试执行')
