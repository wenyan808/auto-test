...
@allure.story('WS订阅K线(sub) 1mon 合约不存在')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestSwapNoti_ws_kline_074:

    @allure.step('前置条件')
    def setup(self):
        print("\n自动化步骤："
              "\n*、sub请求kline ，请求参数中合约不存在；"
              "\n*、验证Kline 1mon返回结果；invalid topic")

    @allure.title('WS订阅K线(sub) 1mon 合约不存在')
    @allure.step('测试执行')
    def test_execute(self):
        with allure.step('详见官方文档'):
            self.contract_code = 'BTC-BTC'  # 不存在的合约
            self.period = '1mon'  # 不存在的period
