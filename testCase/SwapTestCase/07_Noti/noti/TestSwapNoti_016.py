...
步骤/文本
    请求最新成交记录(单个合约，即传参contract_code),可参考文档：https://docs.huobigroup.com/docs/coin_margined_swap/v1/cn/#websocket-3
预期结果
...
class TestSwapNoti_016:
    ...
    def test_execute(self, contract_code):
        with allure.step(
                '请求最新成交记录(单个合约，即传参contract_code),可参考文档：https://docs.huobigroup.com/docs/coin_margined_swap/v1/cn/#websocket-3'):
            # amount\quantity\turnover\direction\price显示正确,不存在Null,[]
            res = api.swap_trade(contract_code)
            pprint(res)
            ...
            trade_record = data[0]
            check_keys = ['amount', 'direction', 'id', 'price', 'quantity', 'contract_code', 'ts']
