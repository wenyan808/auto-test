...
@allure.story('下单区域下止盈止损最优5/10/20档限价单未成交测试')  # 这里填子功能，没有的话就把本行注释掉
class TestCoinswapTriggerOrder_017:
    ...
    def test_execute(self, symbol, symbol_period):
        ...
        with allure.step("查看止盈止损详情"):
            req_tp_sl_detail = current_user.swap_relation_tpsl_order(contract_code=contract_code, order_id=order_id).get("data").get("tpsl_order_info")
            assert len(req_tp_sl_detail) == 2, f"止盈或止损信息缺失: {req_tp_sl_detail}"
            for info in req_tp_sl_detail:
                tp_or_sl = info.get("tpsl_order_type")
                if tp_or_sl == "tp":
                    trigger_type = "ge"
                else:
                    trigger_type = "le"
                expected_info = {"direction": "sell", "trigger_type": trigger_type, "volume": volume, "order_price_type": order_price_type}
                assert common.util.compare_dict(expected_info, info)
