...
class TestContractTriggerOrder_017:
    ...
    def test_execute(self, symbol, symbol_period):
        ...
        with allure.step('9、在下单区域点击买入开多按钮下限价单有结果A'):
            ...
            r_contract_order_history_data = self.c.contract_relation_tpsl_order(
                symbol=symbol, order_id=self.order_id).get("data")
        with allure.step('10、在当前委托-限价委托列表的止盈/止损列点击查看按钮有结果B'):
            expected_info = {"symbol": symbol, "contract_code": contract_code, "contract_type": contract_type, "volume": volume,
    ...
    @allure.step('恢复环境')
    def teardown(self):
        r_cancel = self.c.contract_cancel(
