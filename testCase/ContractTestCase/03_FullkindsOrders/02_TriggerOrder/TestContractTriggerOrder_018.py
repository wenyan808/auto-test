...
class TestContractTriggerOrder_018:
    ...
    def test_execute(self, symbol, symbol_period):
        with allure.step('1、登录交割合约界面'):
            ...
            self.common_user = ContractServiceAPI(url=URL, access_key=COMMON_ACCESS_KEY, secret_key=COMMON_SECRET_KEY)
            #     获取最新价
        ...
        with allure.step('9、在下单区域点击买入开多按钮下限价单有结果A'):
            time.sleep(3)
            r_contract_order_history_data = self.current_user.contract_relation_tpsl_order(symbol=symbol, order_id=order_id).get("data")
            # 获取成交之前的止盈止损单
            before_deal_contract_tpsl_openorders = self.current_user.contract_tpsl_openorders(symbol=symbol, contract_code=self.contract_code).get("data").get("orders")
        with allure.step('10、限价单成交后，在当前委托-止盈止损列表查看下单数据有结果B'):
            ...
            expected_tp_info = {"volume": volume, "direction": direction_tp_sl, "tpsl_order_type": "tp", "trigger_type": "ge", "trigger_price": tp_trigger_price, "order_price": 0, "order_price_type": tp_sl_order_price_type, "status": 1}
            expected_sl_info = {"volume": volume, "direction": direction_tp_sl, "tpsl_order_type": "sl", "trigger_type": "le", "trigger_price": sl_trigger_price, "order_price": 0, "order_price_type": tp_sl_order_price_type, "status": 1}
        ...
        with allure.step("11、做一个卖单促成成交"):
            res_sell = self.common_user.contract_order(symbol=symbol, contract_type=contract_type, contract_code=self.contract_code, price=open_price, volume=volume, direction="sell", offset="open", lever_rate=lever_rate, order_price_type="limit")
            assert res_sell.get("status") == "ok", "下开仓卖单失败: {res_sell}".format(res_sell=res_sell)
            time.sleep(5)
        with allure.step("12. 检查止盈止损单状态"):
            after_contract_tpsl_openorders = self.current_user.contract_tpsl_openorders(symbol=symbol, contract_code=self.contract_code).get('data').get('orders')
            new_orders = [o for o in after_contract_tpsl_openorders if o not in before_deal_contract_tpsl_openorders]
            after_deal_expected_tp_info = {"volume": volume, "direction": direction_tp_sl, "tpsl_order_type": "tp", "trigger_type": "ge", "trigger_price": tp_trigger_price, "order_price": 0, "order_price_type": tp_sl_order_price_type, "status": 2}
            after_deal_expected_sl_info = {"volume": volume, "direction": direction_tp_sl, "tpsl_order_type": "sl", "trigger_type": "le", "trigger_price": sl_trigger_price, "order_price": 0, "order_price_type": tp_sl_order_price_type, "status": 2}
            for tp_sl in new_orders:
                if tp_sl.get("tpsl_order_type") == "tp":
                    assert common.util.compare_dict(after_deal_expected_tp_info, tp_sl)
                else:
