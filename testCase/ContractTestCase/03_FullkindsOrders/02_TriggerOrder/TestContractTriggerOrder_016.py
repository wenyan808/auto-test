...
class TestContractTriggerOrder_016:
    ...
    def test_execute(self, symbol, symbol_period):
        ...
        with allure.step('2、选择BTC当周，选择杠杆5X，点击开仓-限价按钮'):
            ...
            contract_ltc_info = self.c.contract_contract_info(
        ...
        with allure.step('9、在下单区域点击买入开多按钮下限价单有结果A'):
            time.sleep(3)
            r_contract_order_history_data = self.c.contract_relation_tpsl_order(
                symbol=symbol, order_id=order_id).get("data")
            # 获取成交之前的止盈止损单
            before_deal_contract_tpsl_openorders = self.c.contract_tpsl_openorders(
        ...
        with allure.step('10、在当前委托-限价委托列表的止盈/止损列点击查看按钮有结果B'):
            ...
            expected_tp_info = {"volume": volume, "direction": direction_tp_sl, "tpsl_order_type": "tp", "trigger_type": "ge",
                                "trigger_price": tp_trigger_price, "order_price": tp_trigger_price, "order_price_type": order_price_type, "status": 1}
            expected_sl_info = {"volume": volume, "direction": direction_tp_sl, "tpsl_order_type": "sl", "trigger_type": "le",
                                "trigger_price": sl_trigger_price, "order_price": sl_trigger_price, "order_price_type": order_price_type, "status": 1}
        ...
        with allure.step("11、做一个卖单促成成交"):
            res_sell = self.c.contract_order(symbol=symbol, contract_type=contract_type, contract_code=self.contract_code,
        ...
        with allure.step("12. 检查止盈止损单状态"):
            ...
            after_deal_expected_tp_info = {"volume": volume, "direction": direction_tp_sl, "tpsl_order_type": "tp", "trigger_type": "ge",
                                           "trigger_price": tp_trigger_price, "order_price": tp_trigger_price, "order_price_type": order_price_type, "status": 2}
            after_deal_expected_sl_info = {"volume": volume, "direction": direction_tp_sl, "tpsl_order_type": "sl", "trigger_type": "le",
                                           "trigger_price": sl_trigger_price, "order_price": sl_trigger_price, "order_price_type": order_price_type, "status": 2}
    ...
    @allure.step('环境清理')
    def teardown(self):
        r_cancel_all = self.c.contract_tpsl_cancelall(
