...
@allure.feature('功能')
@pytest.mark.stable
class TestContractLimitOrder_005:
    ...
    def test_contract_limit_order(self, symbol, symbol_period):
        ...
        if not asks:
            ...
            assert r_common_sell.get(
                "status") == "ok", f"通用账号下卖单失败: {r_common_sell}"
            r_trend_req = contract_api.contract_depth(
                symbol=symbol_period, type="step5")
            asks = r_trend_req.get("tick").get("asks")
        ...
        r_order_buy = contract_api.contract_order(symbol=symbol, contract_type='this_week', price=lower_price,
                                                  volume='1',
        ...
        for order in all_orders:
            ...
            if current_order_id == generated_order_id:
                expected_info_dic = {"status": 7, "lever_rate": 5,
                                     "order_type": 1, "volume": 1, "price": lower_price}
