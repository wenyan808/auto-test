...
class TestContractLimitOrder_009:
    ...
    def test_contract_limit_order(self, symbol, symbol_period):
        ...
        if current_asks:
            ...
            r_eat = service.contract_order(symbol=symbol, contract_type='this_week', price=highest_price, volume=total_asks, direction='buy', offset='open', lever_rate=lever_rate, order_price_type='limit')
            assert r_eat.get("status") == "ok", f"吃卖单失败: r{r_eat}"
            time.sleep(3)
        ...
        pprint("\n步骤四: 以对手价买入开多\n")
        r_buy_opponent = contract_api.contract_order(symbol=symbol, contract_type='this_week', order_price_type='opponent', price="", direction="buy", offset="open", lever_rate=lever_rate, volume=1)
