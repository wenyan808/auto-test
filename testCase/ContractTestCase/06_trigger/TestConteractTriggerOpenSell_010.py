...
# @Author : 
    ...
    优先级
        2
    用例别名
        TestConteractTriggerOpenSell_010
"""
...
class TestConteractTriggerOpenSell_010:

    @allure.step('前置条件')
    def setup(self):
        print('''  ''')

    @allure.title('计划委托卖出开空卖出量超过后台设置限制')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period):
        ...
        with allure.step('4、输入卖出价（如：45000）'):
            pass
        with allure.step('5、输入卖出量10000张，后台设置6000张'):
            pass
        with allure.step('6、点击卖出开空按钮'):
            pass
