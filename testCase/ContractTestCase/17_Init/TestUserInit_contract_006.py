...
预期结果
    ...
    2、检查APO中数据，存在持仓、资金key  
      ...
      Position:#BCH#BCH211231#2                （交割有几个合约就要查几个合约，当周/次周/当季/次季
      orderPositionFrozen:BCH211231#1        （交割有几个合约就要查几个合约，当周/次周/当季/次季）
      orderPositionFrozen:BCH211231#2        （交割有几个合约就要查几个合约，当周/次周/当季/次季）
      clearUnPositionFrozen:BCH211231#1      （交割有几个合约就要查几个合约，当周/次周/当季/次季）
      clearUnPositionFrozen:BCH211231#2      （交割有几个合约就要查几个合约，当周/次周/当季/次季）    
       Account:#BCH 
...
class TestUserInit_contract_006:
