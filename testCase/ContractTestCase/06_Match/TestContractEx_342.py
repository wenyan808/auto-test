...
# @Author : 
    ...
    用例别名
         用例别名
        TestContractEx_342 撮合当季
        TestContractEx_476 撮合次季
"""
...
class TestContractEx_342:
    ids = ['TestContractEx_342', 'TestContractEx_476']
    datas = [('当季 卖出开仓部分成交 撤单', 'quarter'), ('次季 卖出开仓部分成交 撤单', 'next_quarter')]

    @allure.step('测试执行')
    @pytest.mark.flaky(reruns=3, reruns_delay=3)
    @pytest.mark.parametrize('case_name,contest_type',datas,ids=ids)
    def test_execute(self,symbol,case_name,contest_type):
        with allure.step('详见官方文档'):
            ...
            user01.contract_order(symbol=symbol, contract_code=self.contract_code, volume=1,
