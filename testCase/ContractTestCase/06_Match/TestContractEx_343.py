...
# @Author : 
    ...
    优先级
        0
    用例别名
        TestContractEx_343 撮合当季
        TestContractEx_477 撮合次季
"""
...
class TestContractEx_343:
    ids = ['TestContractEx_343', 'TestContractEx_477']
    datas = [('当季 买入平仓部分成交 撤单', 'quarter'), ('次季 买入平仓部分成交 撤单', 'next_quarter')]
    ...
    def test_execute(self,symbol,case_name,contest_type):
        with allure.step('详见官方文档'):
            ...
            orderInfo = user01.contract_order(symbol=symbol,contract_code=self.contract_code,
                                              price=round(self.currentPrice, 2),
                                              contract_type=self.contract_type,direction='buy',offset='close',volume=2)
