...
class TestContractEx_249:
    ...
    def setup(self, symbol, symbol_period):
        ...
        print(res1)
        res2 = contract_api.contract_order(symbol=symbol, contract_type=contract_type, price=self.current,
                                           volume=10, direction="sell", offset="open", order_price_type='limit', lever_rate=5)
        print(res2)

    @allure.title('撮合次周 买入平仓 全部成交单人多笔价格不同的订单')
    @allure.step('测试执行')
