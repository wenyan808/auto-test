...
class TestContractEx_105:
    ...
    @allure.title('撮合当周 买入开仓 全部成交多人多笔价格相同的订单     ')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period, DB_orderSeq):
        with allure.step('详见官方文档'):
            ...
            res = ATP.current_user_make_order(
                contract_code=symbol_period, price=self.current, volume=5, direction="sell", offset="open")
            pprint(res)
            ...
            pprint(buy_order1)
            buy_order2 = ATP.common_user_make_order(contract_code=symbol_period, price=self.current, volume=2,
