...
class TestContractEx_125:
    ...
    def setup(self, symbol, symbol_period):
        ...
        res2 = contract_api.contract_order(
            symbol=symbol, contract_type=contract_type, price=self.current, volume=10, direction="buy", offset="open", order_price_type='limit', lever_rate=5)
        print(res2)
        ATP.make_market_depth(depth_count=5)

    @allure.title('撮合当周 闪电平仓最优30档 卖出')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period, DB_orderSeq):
        with allure.step('详见官方文档'):
            ...
            res1 = common_contract_api.contract_order(symbol=symbol, contract_type=contract_type, price=round(self.current *
                                                      1.01, 2), volume=5, direction="buy", offset="close", order_price_type='limit', lever_rate=5)
            print(res1)

            order = contract_api.lightning_close_position(
                symbol=symbol, contract_type=contract_type, volume=2, direction="sell", order_price_type='lightning')
            print(order)
