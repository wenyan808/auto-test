...
class TestContractEx_110:
    ...
    @allure.title('撮合当周 卖出开仓 部分成交单人多笔价格不同的订单  ')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period, DB_orderSeq):
        with allure.step('详见官方文档'):
            ...
            res1 = common_contract_api.contract_order(symbol=symbol, contract_type=contract_type, price=round(self.current *
                                                      1.01, 2), volume=2, direction="buy", offset="open", order_price_type='limit', lever_rate=5)
            print(res1)
