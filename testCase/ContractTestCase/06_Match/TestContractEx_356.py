...
class TestContractEx_356:
    ids = ['TestContractEx_356',
           'TestContractEx_360',
           'TestContractEx_490',
           'TestContractEx_494']

    datas = [('quarter','当季 部分成交 卖出平仓',1),
             ('quarter', '当季 全部成交 卖出平仓',2),
             ('next_quarter', '次季 部分成交 卖出平仓',1),
             ('next_quarter', '次季 全部成交 卖出平仓',2)]


    @allure.step('测试执行')
    @pytest.mark.flaky(reruns=3, reruns_delay=3)
    @pytest.mark.parametrize('contest_type,caseName,volume',datas,ids=ids)
    def test_execute(self,symbol,contest_type,caseName,volume):
