...
步骤/文本
    1、打开MGT后台管理系统
    2、点击财务-财务工具-平账，流水类型选择（系统借贷账户），平种标识如（XRP），输入金额，备注
    3、输入用户的UID以及金额
    4、点击‘提交申请’按钮
...
class TestContractMGTtransfer_040:
    ...
    @allure.title('结算中-系统借贷账户')
    @allure.step('测试执行')
    def test_execute(self):
        with allure.step('打开MGT后台管理系统点击财务-财务工具-平账，流水类型选择（系统借贷账户），平种标识如（XRP），输入金额'):
            params = ["XRP", {
                "productId": "XRP",
                "flatAccount": 3,
                "uid": None,
