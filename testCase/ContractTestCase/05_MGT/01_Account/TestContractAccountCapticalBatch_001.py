...
@pytest.mark.stable
class TestContractAccountCapticalBatch_001:
    ...
    @allure.title('平台流水表-每日跑批:校验平台资产')
    @allure.step('测试执行')
    def test_execute(self):
        ...
        with allure.step('查询各流水类型的DB数据,接口数据与DB数据进行对比'):
            ...
            for key, value in capital_param_dic.items():
                sqlStr = 'SELECT TRUNCATE(SUM(money),8) as money FROM t_account_action WHERE create_time > {} AND create_time <= {} AND money_type = {} AND product_id = "{}"'.format(
            ...
            sqlStr = 'SELECT SUM(money) as money FROM t_flat_money_record WHERE flat_time > {} AND flat_time <= {} AND flat_account = {} AND product_id = "{}"'.format(
                beginDateUnixTime, endDateUnixTime, 11, symbol)
            print(sqlStr)
            rec_dict_tuples = contract_conn.contract_selectdb_execute(
                db='btc', sqlStr=sqlStr)
            if rec_dict_tuples[0]["money"]:
