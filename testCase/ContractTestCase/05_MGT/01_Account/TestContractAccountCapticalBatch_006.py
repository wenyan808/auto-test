...
@pytest.mark.stable
class TestContractAccountCapticalBatch_006:
    ...
    @allure.title('平台流水表-每日跑批:校验运营活动')
    @allure.step('测试执行')
    def test_execute(self):
        ...
        with allure.step('查询各流水类型的DB数据,接口数据与DB数据进行对比'):
            ...
            symbol = 'btc'
            # money_type 5:开仓手续费-taker,6:开仓手续费-maker,7:平仓手续费-taker,8:平仓手续费-maker,11:运营活动,14:币币转入,15:币币转出,20:平账,21:借贷转运营,22:运营转借贷,23:手续费转运营,24:注入到爆仓,25:从爆仓提取,26:给用户赠币-赔偿,27:扣减用户资产-惩戒,28:活动奖励,29:返利
            ...
            # 运营活动验证如下:money_type:借贷转运营(21),运营转借贷(22),注入到爆仓（24）,从爆仓提取（25）,给用户赠币-赔偿（26）,扣减用户资产-惩戒（27）,手续费转运营(23),活动奖励（28）,返利（29）,平账（20）
            capital_param_dic = {"borrowToOperate": 21,
                                 "operateToBorrow": 22,
                                 "toBurst": 24,
                                 "fromBurst": 25,
                                 "compensate": 26,
