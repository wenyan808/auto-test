...
@pytest.mark.stable
class TestContractAccountCapticalBatch_026:
    ...
    @allure.title('平台流水表-结算对账:校验平台资产')
    @allure.step('测试执行')
    def test_execute(self):
        ...
        with allure.step('查询各流水类型的DB数据,接口数据与DB数据进行对比'):
            ...
            # 平台资产验证如下:money_type:币币转入（14）,币币转出（15）,平账（20）
            capital_param_dic = {"moneyIn": 14, "moneyOut": 15}
            currInterest = 0  # 流水当期发生

            for key, value in capital_param_dic.items():
                sqlStr = 'SELECT SUM(money) as money FROM t_account_action WHERE create_time > {} AND create_time <= {} AND money_type = {} AND product_id = "{}"'.format(
            ...
            # 平账流水单独从平账表获取(仅限平台资产从此表查询,其它不用)
            sqlStr = 'SELECT SUM(money) as money FROM t_flat_money_record WHERE flat_time > {} AND flat_time <= {} AND flat_account = {} AND product_id = "{}"'.format(
                beginDateTime, endDateTime, value, symbol)
            print(sqlStr)
            rec_dict_tuples = contract_conn.selectdb_execute(
                db='btc', sqlStr=sqlStr)
            if rec_dict_tuples[0]["money"]:
                ...
                money = rec_dict_tuples[0]["money"]
                assert money == Decimal(
                    platform_capital_dic["flatMoney"]), "flatMoney 流水比对异常"
