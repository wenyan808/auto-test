...
@pytest.mark.stable
class TestContractAccountCapticalBatch_005:
    ...
    @allure.title('平台流水表-每日跑批:校验交割手续费')
    @allure.step('测试执行')
    def test_execute(self, symbol):
        ...
        with allure.step('查询各流水类型的DB数据,接口数据与DB数据进行对比'):
            ...
            # 交割手续费验证如下:money_type:手续费转运营(23),交割手续费（11）,平账（20）
            capital_param_dic = {
                "feeToOperate": 23,
                "deliveFee": 11,
                "flatMoney": 20,
            ...
            for key, value in capital_param_dic.items():
                sqlStr = 'SELECT SUM(money) as money FROM t_account_action WHERE create_time > {} AND create_time <= {} AND money_type = {} AND product_id = "{}" AND user_id=193800'.format(
