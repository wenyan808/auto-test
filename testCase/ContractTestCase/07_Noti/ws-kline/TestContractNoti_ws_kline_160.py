...
class TestContractNoti_ws_kline_160:
    ...
    def test_execute(self, symbol, symbol_period):
        with allure.step('详见官方文档'):
            period = "1day"
            result = contract_service_ws.contract_req_kline(contract_code=symbol_period, period=None,
                                                            From=self.from_time, to=self.to_time)
            pprint(result)
            errmsg = result['err-msg']
            if errmsg != "invalided kline type":
                assert False
