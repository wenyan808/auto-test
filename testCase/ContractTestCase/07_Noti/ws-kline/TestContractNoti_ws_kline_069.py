...
class TestContractNoti_ws_kline_069:
    ...
    def setup(self, symbol):
        ...
        sell_price = round(self.current_price * 1.01, 2)
        buy_price = round(self.current_price * 1.01, 2)
        ATP.common_user_make_order(price=sell_price, direction='sell')
        ATP.common_user_make_order(price=buy_price, direction='buy')
        time.sleep(1)

    @allure.title('WS订阅K线(sub) 30min')
    @allure.step('测试执行')
