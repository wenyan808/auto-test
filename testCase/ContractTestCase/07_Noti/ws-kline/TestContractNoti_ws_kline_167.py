...
class TestContractNoti_ws_kline_167:
    ...
    @allure.title('WS订阅K线(req) period大小写')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period):
        with allure.step('详见官方文档'):
            period = "1day"
            periodfirst = period[:-2].upper()
            peroidend = period[-2:].lower()
            result = contract_service_ws.contract_req_kline(contract_code=symbol_period, period=periodfirst+peroidend,
