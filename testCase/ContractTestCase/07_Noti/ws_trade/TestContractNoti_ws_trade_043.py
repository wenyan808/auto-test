...
class TestContractNoti_ws_trade_043:
    ...
    def setup(self, symbol):
        ...
        ATP.common_user_make_order(
            price=sell_price, order_price_type='optimal_20',  direction='sell')
        ATP.common_user_make_order(
            price=buy_price, order_price_type='optimal_20', direction='buy')
        time.sleep(0.5)
        self.current_price = ATP.get_current_price()

    @allure.title('WS订阅成交(req)  最优20档成交')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period):
        with allure.step('详见官方文档'):
            result = contract_service_ws.contract_req_tradedetail(
                symbol_period)
            pprint(result)
