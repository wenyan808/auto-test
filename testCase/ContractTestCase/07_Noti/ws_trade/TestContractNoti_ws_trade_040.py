...
class TestContractNoti_ws_trade_040:
    ...
    def setup(self, symbol):
        ...
        ATP.common_user_make_order(
            price=sell_price, order_price_type='opponent',  direction='sell')
        ATP.common_user_make_order(
            price=buy_price, order_price_type='opponent', direction='buy')
        time.sleep(0.5)
        self.current_price = ATP.get_current_price()

    @allure.title('WS订阅成交(req)  对手价成交')
    @allure.step('测试执行')
