...
# @Author : 
    ...
    优先级
        3
    用例别名
        TestContractNoti_restful_kline_043
"""
...
class TestContractNoti_restful_kline_043:
    ...
    @allure.title('restful请求K线 60min to传参错误 非时间戳')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period):
        with allure.step('详见官方文档'):
            period = "60min"
            result = contract_api.contract_kline(symbol=symbol_period, period=period, size="2", to="abcd")
            pprint(result)
