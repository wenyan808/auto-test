...
# @Author : chenwei
    用例标题
        restful请求K线 1week 传参from,to,size
    前置条件
    ...
    优先级
        1
    用例别名
        TestContractNoti_restful_kline_066
"""
...
class TestContractNoti_restful_kline_066:
    ...
    @allure.title('restful请求K线 1week 传参from,to,size')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period):
        with allure.step('详见官方文档'):
            period = "1week"
            result = contract_api.contract_kline(symbol=symbol_period, period=period, size="2", From=self.from_time, to=self.to_time)
            pprint(result)
