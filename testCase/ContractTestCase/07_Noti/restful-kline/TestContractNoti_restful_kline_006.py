...
# @Author : 
    ...
    优先级
        3
    用例别名
        TestContractNoti_restful_kline_006
"""
...
class TestContractNoti_restful_kline_006:
    ...
    @allure.title('restful请求K线1min from传参错误 非时间戳')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period):
        with allure.step('详见官方文档'):
            period = "1min"
            result = contract_api.contract_kline(symbol=symbol_period, period=period, size="2", From="abc", to=self.to_time)
            pprint(result)
