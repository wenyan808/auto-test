...
# @Author : chenwei
    ...
    优先级
        2
    用例别名
        TestContractNoti_restful_kline_067
"""
...
class TestContractNoti_restful_kline_067:
    ...
    @allure.title('restful请求K线 1week 只传from,不传to')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period):
        with allure.step('详见官方文档'):
            period = "1week"
            result = contract_api.contract_kline(symbol=symbol_period, period=period, size="2", From=self.from_time)
            pprint(result)
