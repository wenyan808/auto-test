...
# @Author : 
    ...
    优先级
        2
    用例别名
        TestContractNoti_restful_kline_005
"""
...
@pytest.mark.stable
class TestContractNoti_restful_kline_005:
    ...
    @allure.title('restful请求K线1min 只传to,不传from')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period):
        with allure.step('详见官方文档'):
            period = "1min"
            result = contract_api.contract_kline(symbol=symbol_period, period=period, size="2", to=self.to_time)
            pprint(result)
