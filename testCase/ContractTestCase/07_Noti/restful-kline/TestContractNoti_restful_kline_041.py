...
# @Author : chenwei
    ...
    优先级
        2
    用例别名
        TestContractNoti_restful_kline_041
"""
...
@pytest.mark.stable
class TestContractNoti_restful_kline_041:
    ...
    @allure.title('restful请求K线 60min 只传to,不传from')
    @allure.step('测试执行')
    def test_execute(self, symbol, symbol_period):
        with allure.step('详见官方文档'):
            period = "60min"
            result = contract_api.contract_kline(symbol=symbol_period, period=period, size="2", to=self.from_time)
            pprint(result)
