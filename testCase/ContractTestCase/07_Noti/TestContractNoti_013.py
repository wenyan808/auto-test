...
步骤/文本
    请求聚合行情(单个合约，即传参contract_code),可参考文档：https://docs.huobigroup.com/docs/dm/v1/cn/#websocket-3
预期结果
    open、close、low、high价格正确；amount、vol、count值正确asks、bid值正确,不存在Null,[]
优先级
    0
用例编号
    TestContractNoti_013
自动化作者
    韩东林
"""
...
class TestContractNoti_013:
    ...
    def test_execute(self, symbol, symbol_period):
        with allure.step('请求聚合行情(单个合约，即传参contract_code),可参考文档：https://docs.huobigroup.com/docs/dm/v1/cn/#websocket-3'):
            ...
            ch = res.get('ch', '')
            assert ch == f'market.{symbol_period.upper()}.detail.merged', 'ch 不正确'
