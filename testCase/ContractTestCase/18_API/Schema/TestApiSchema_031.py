...
步骤/文本
    1.调用接口：api/v1/contract_position_limit
预期结果
...
@allure.story('获取用户的合约持仓量限制')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : Alex Li')
@pytest.mark.stable
class TestApiSchema_031:
    ...
    def test_execute(self, sub_uid):
        with allure.step('1、调用接口：api/v1/contract_position_limit'):
            pass
        with allure.step('2、接口返回的json格式、字段名、字段值正确'):
            ...
            res = contract_api.contract_position_limit(symbol="BTC")
            print(res)
            assert res['status'] == 'ok'
            assert common.util.compare_dictkey(["symbol", "list"], res.data[0])
            assert common.util.compare_dictkey(
                ["contract_type", "buy_limit", "sell_limit"], res.data[0].list[0])
