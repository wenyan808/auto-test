...
步骤/文本
    1.调用接口：api/v1/contract_risk_info
预期结果
...
@allure.story('获取风险准备金和预估分摊系数')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : Alex Li')
@pytest.mark.stable
class TestApiSchema_006:
    ...
    def test_execute(self):
        with allure.step('1、调用接口：api/v1/contract_risk_info'):
            pass
        with allure.step('2、接口返回的json格式、字段名、字段值正确'):
            ...
            res = contract_api.contract_risk_info(symbol='BTC')
            print(res)
            assert res['status'] == 'ok'
            assert common.util.compare_dictkey(
                ["symbol", "insurance_fund", "estimated_clawback"], res.data[0])
