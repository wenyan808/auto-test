...
所属分组
    定序,清算,落库,复核
用例标题
    次周合约下单挂单数据，检查清算后的redis的APO中的订单数据
前置条件

步骤/文本
    1.用户下次周合约的挂单，记住user_order_id
预期结果
...
@allure.story('次周合约下单挂单数据，检查清算后的redis的APO中的订单数据')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : Alex Li')
@pytest.mark.stable
class TestClear_contract_002:
    ...
    def test_execute(self, symbol_period):
        with allure.step('1、用户下次周合约的挂单，记住user_order_id'):
            pass
        with allure.step('2、根据user_order_id核对APO中持仓数据（注意下面的user_order_id替换成实际下单的数据）Order:#847833760893657088#1  （1为多单，2为空单）'):
            ...
            res = contract_api.contract_order(
                symbol='BTC', contract_type='next_week', price=current_price, volume=1, direction='buy', offset='open')
            print(res)
