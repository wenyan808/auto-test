...
所属分组
    定序,清算,落库,复核
用例标题
    次周合约下单撤单数据，检查清算后的redis的APO中的订单数据
前置条件

步骤/文本
    1.用户下次周合约的撤单，记住user_order_id
预期结果
...
@allure.story('次周合约下单撤单数据，检查清算后的redis的APO中的订单数据')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : Alex Li')
@pytest.mark.stable
class TestClear_contract_006:
    ...
    def test_execute(self, symbol_period, symbol):
        ...
        with allure.step('1、用户下次周合约的撤单，记住user_order_id'):
            pass
