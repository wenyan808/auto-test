...
class TestLinearTriggerOpenSell_006:
    ...
    def setup(self, symbol, contract_code, lever_rate, directionB, directionS, offsetC, offsetO):
        print('\n测试标题：触发价等于最新价场景，验证计划委托开空单-委托触发'
              '\n*、先清盘避免盘口数据干扰;'
              '\n*、触发价等于最新价，买入价低于最新价下开空计划委托单；'
              '\n*、触发计划委托订单；'
              '\n*、验证计划委托订单触发否')
    ...
    def test_execute(self):
        ...
        with allure.step('2、计划委托下单，触发价为50000；买入价为49800；'):
            self.trigger_price = self.currentPrice
        ...
        with allure.step('3、行情-最新价更新；使最新价达到50000价，触发计划委托单转换为限制单'):
            linear_api.linear_order(contract_code=self.contract_code, price=self.currentPrice, order_price_type='limit',
