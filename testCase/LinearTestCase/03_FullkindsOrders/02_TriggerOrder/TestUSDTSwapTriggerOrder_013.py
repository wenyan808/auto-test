...
class TestUSDTSwapTriggerOrder_013:
    ...
    def test_execute(self, symbol):
        ...
        with allure.step('9、检查历史委托-计划委托界面有结果D'):
            trigger_his_orders = self.current_user.linear_trigger_hisorders(contract_code=self.contract_code, status="6", trade_type=3, create_date=7).get("data").get("orders")
            ...
            expected_info = {"contract_code": self.contract_code, "trigger_type": "ge", "margin_mode": "isolated", "volume": 10, "direction": "buy", "offset": "close", "lever_rate": 5, "status": 6, "trigger_price": trigger_price, "order_price_type": "limit"}
