...
class TestUSDTSwapTriggerOrder_017:
    ...
    def test_execute(self, symbol):
        ...
        with allure.step('10、在当前委托-限价委托列表的止盈/止损列点击查看按钮有结果B'):
            tp_sl_actual = self.current_user.linear_relation_tpsl_order(contract_code=self.contract_code,
                                                                        order_id=tp_sl_order_id).get("data")
            expected_tp_sl_info = {"contract_code": self.contract_code, "volume": 10, "price": price,
                                   "order_price_type": "limit", "direction": "buy", "offset": "open"}
            ...
            for i in tp_sl_detail_actual:
                ...
                expected_tp_sl_each = {"volume": 10, "direction": "sell", "trigger_price": trigger_price,
                                       "trigger_type": trigger_type, "order_price": 0, "status": 1,
                                       "order_price_type": "optimal_5"}
