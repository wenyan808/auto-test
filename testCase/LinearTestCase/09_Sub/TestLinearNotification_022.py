...
@allure.story('订阅全仓订单撮合（有订单撮合）')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : 柳攀峰')
@pytest.mark.stable
class TestLinearNotification_022:
    ...
    @allure.title('订阅全仓订单撮合（有订单撮合）')
    @allure.step('测试执行')
    def test_execute(self):
        contract_code = 'BTC-USDT'
        with allure.step('1、订阅全仓订单撮合（有订单撮合）'):
            sub = {
                "op": "sub",
                "cid": 11538447,
                "topic": "matchOrders_cross.{}".format(contract_code)
            }
