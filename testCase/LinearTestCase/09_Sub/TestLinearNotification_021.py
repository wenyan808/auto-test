...
@allure.story('订阅全仓订单成交（有订单成交）')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : 柳攀峰')
# @pytest.mark.stable
class TestLinearNotification_021:
    ...
    @allure.title('订阅全仓订单成交（有订单成交）')
    @allure.step('测试执行')
    def test_execute(self):
        contract_code = 'BTC-USDT'
        with allure.step('1、订阅全仓订单成交（有订单成交）'):
            sub = {
                "op": "sub",
                "cid": 11538447,
                "topic": "orders_cross.{}".format(contract_code)
            }
