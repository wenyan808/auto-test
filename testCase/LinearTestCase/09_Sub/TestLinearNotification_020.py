...
@allure.story('订阅全仓资金变动（有资金变动）')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : 柳攀峰')
# @pytest.mark.stable
class TestLinearNotification_020:
    ...
    @allure.title('订阅全仓资金变动（有资金变动）')
    @allure.step('测试执行')
    def test_execute(self):
        contract_code = 'BTC-USDT'
        with allure.step('1、订阅全仓资金变动（有资金变动）'):
            sub = {
                "op": "sub",
                "cid": 11538447,
                "topic": "accounts_cross.{}".format("USDT")
            }
            ...
            assert result["op"] == "notify" and result["topic"] == "accounts_cross" and len(
                result["data"]) > 0
