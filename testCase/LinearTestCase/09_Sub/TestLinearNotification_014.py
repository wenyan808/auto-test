...
@allure.story('订阅逐仓持仓变动（有持仓变动）')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : 柳攀峰')
# @pytest.mark.stable
class TestLinearNotification_014:
    ...
    @allure.title('订阅逐仓持仓变动（有持仓变动）')
    @allure.step('测试执行')
    def test_execute(self):
        contract_code = 'BSV-USDT'
        with allure.step('1、订阅逐仓持仓变动（有持仓变动）'):
            sub = {
                "op": "sub",
                "cid": 11538447,
            ...
            assert result["op"] == "notify" and result["topic"] == "positions" and len(
                result["data"]) > 0
