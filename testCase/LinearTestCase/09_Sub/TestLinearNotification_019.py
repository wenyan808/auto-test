...
@allure.story('订阅全仓持仓变动（有持仓变动）')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : 柳攀峰')
# @pytest.mark.stable
class TestLinearNotification_019:
    ...
    @allure.title('订阅全仓持仓变动（有持仓变动）')
    @allure.step('测试执行')
    def test_execute(self):
        contract_code = 'BTC-USDT'
        with allure.step('1、订阅全仓持仓变动（有持仓变动）'):
            sub = {
                "op": "sub",
                "cid": 11538447,
                "topic": "positions_cross.{}".format(contract_code)
            }
            ...
            time.sleep(1)
            pprint(result)
            assert result["op"] == "notify" and result["topic"] == "positions_cross" and len(
                result["data"]) > 0
