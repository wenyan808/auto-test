...
@allure.story('订阅强平订单（无强平订单）')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : 柳攀峰')
@pytest.mark.stable
class TestLinearNotification_011:
    ...
    @allure.title('订阅强平订单（无强平订单）')
    @allure.step('测试执行')
    def test_execute(self):
        contract_code = 'BTC-USDT'
        with allure.step('1、订阅强平订单（无强平订单）'):
            sub = {
                "op": "sub",
                "cid": 11538447,
                "topic": "public.{}.liquidation_orders".format(contract_code)
            }
            result = linear_service_ws.linear_notification(sub)
            assert result["err-code"] == 0
