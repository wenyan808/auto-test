...
@allure.story('订阅全仓计划委托（有计划委托）')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : 柳攀峰')
# @pytest.mark.stable
class TestLinearNotification_023:
    ...
    @allure.title('订阅全仓计划委托（有计划委托）')
    @allure.step('测试执行')
    def test_execute(self):
        contract_code = 'BTC-USDT'
        with allure.step('1、订阅全仓计划委托（有计划委托）'):
            sub = {
                "op": "sub",
                "cid": 11538447,
                "topic": "trigger_order_cross.{}".format(contract_code)
            }
            ...
            res = ATP.current_user_make_trigger_order(order_price_type=order_price_type, contract_code=contract_code,
                                                      order_price=current, trigger_price=current, volume=1, direction=direction, offset=offset)
            pprint(res)
