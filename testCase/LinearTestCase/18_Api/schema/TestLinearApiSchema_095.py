...
# @Author : 
    ...
    步骤/文本
        调用接口：/linear-swap-api/v1/swap_cross_lightning_close_position
    预期结果
        A)接口返回的json格式、字段名、字段值正确
    优先级
        0
    用例别名
        TestLinearApiSchema_095
"""
import time
...
class TestLinearApiSchema_095:
    ...
    def test_execute(self, contract_code, symbol):
        with allure.step('调用接口：/linear-swap-api/v1/swap_cross_lightning_close_position'):
            ...
            r = linear_api.linear_cross_lightning_close_position(contract_code=contract_code,
                                                                 volume='1',
                                                                 direction='sell',
                                                                 client_order_id='',
