...
# @Author : 
    ...
    步骤/文本
        调用接口：/linear-swap-api/v1/swap_cancel
    预期结果
        A)接口返回的json格式、字段名、字段值正确
    优先级
        0
    用例别名
        TestLinearApiSchema_046
"""
import time
...
@pytest.mark.stable
class TestLinearApiSchema_046:

    @allure.step('前置条件')
    def setup(self):
        self.price = atp.ATP.get_adjust_price(rate=0.98)

    @allure.title('撤销合约订单（逐仓）')
    @allure.step('测试执行')
    def test_execute(self, contract_code, symbol):
        with allure.step('调用接口：/linear-swap-api/v1/swap_cancelall'):
            ...
            r = linear_api.linear_cancel(contract_code=contract_code,
                                         order_id=order_id)
            pprint(r)
            schema = {
                'data': {
