...
# @Author : 
    ...
    步骤/文本
        调用接口：/linear-swap-api/v1/swap_tpsl_cancel
    预期结果
        A)接口返回的json格式、字段名、字段值正确
    优先级
        0
    用例别名
        TestLinearApiSchema_063
"""
import time
...
@pytest.mark.stable
class TestLinearApiSchema_063:

    @allure.step('前置条件')
    def setup(self):
        pass

    @allure.title('止盈止损撤单（逐仓）')
    @allure.step('测试执行')
    def test_execute(self, contract_code, symbol):
        with allure.step('调用接口：/linear-swap-api/v1/swap_tpsl_cancel'):
            ...
            r = linear_api.linear_tpsl_cancel(contract_code=contract_code,
                                              order_id=order_id)
            pprint(r)
