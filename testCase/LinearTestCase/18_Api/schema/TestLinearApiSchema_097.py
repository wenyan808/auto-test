...
# @Author : 
    ...
    步骤/文本
        调用接口：/linear-swap-api/v1/swap_cross_trigger_cancel
    预期结果
        A)接口返回的json格式、字段名、字段值正确
    优先级
        0
    用例别名
        TestLinearApiSchema_097
"""
import time
...
class TestLinearApiSchema_097:
    ...
    def test_execute(self, contract_code, symbol):
        with allure.step('调用接口：/linear-swap-api/v1/swap_cross_trigger_cancel'):
            ...
            r = linear_api.linear_cross_trigger_cancel(contract_code=contract_code,
                                                       order_id=order_id)
            pprint(r)
            schema = {
                'data': {
