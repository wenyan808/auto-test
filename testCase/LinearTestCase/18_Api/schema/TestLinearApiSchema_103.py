...
# @Author : 
    ...
    步骤/文本
        调用接口：/linear-swap-api/v1/swap_cross_tpsl_cancelall
    预期结果
        A)接口返回的json格式、字段名、字段值正确
    优先级
        0
    用例别名
        TestLinearApiSchema_103
"""
import time
...
@pytest.mark.stable
class TestLinearApiSchema_103:

    @allure.step('前置条件')
    def setup(self):
        pass

    @allure.title('止盈止损全部撤单（全仓）')
    @allure.step('测试执行')
    def test_execute(self, contract_code, symbol):
        with allure.step('调用接口：/linear-swap-api/v1/swap_cross_tpsl_cancelall'):
