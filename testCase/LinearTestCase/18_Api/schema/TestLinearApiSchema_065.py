...
# @Author : 
    ...
    步骤/文本
        调用接口：/linear-swap-api/v1/swap_tpsl_openorders
    预期结果
        A)接口返回的json格式、字段名、字段值正确
    优先级
        0
    用例别名
        TestLinearApiSchema_065
"""
import time
...
@pytest.mark.stable
class TestLinearApiSchema_065:

    @allure.step('前置条件')
    def setup(self):
        pass

    @allure.title('获取止盈止损当前委托（逐仓）')
    @allure.step('测试执行')
    def test_execute(self, contract_code, symbol):
        with allure.step('调用接口：/linear-swap-api/v1/swap_tpsl_openorders'):
            ...
            time.sleep(2)
            r = linear_api.linear_tpsl_order(contract_code=contract_code,
                                             direction='sell',
                                             volume='1',
