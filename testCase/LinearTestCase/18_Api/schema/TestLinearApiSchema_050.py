...
# @Author : 
    ...
    步骤/文本
        调用接口：/linear-swap-api/v1/swap_order_detail
    预期结果
        A)接口返回的json格式、字段名、字段值正确
    优先级
        0
    用例别名
        TestLinearApiSchema_050
"""
import time
...
@pytest.mark.stable
class TestLinearApiSchema_050:  # 疑似bug，trades的值为[]
    ...
    def test_execute(self, contract_code, symbol):
        with allure.step('调用接口：/linear-swap-api/v1/swap_order_detail'):
            ...
            r = linear_api.linear_order_detail(contract_code=contract_code,
                                               order_id=order_id,
