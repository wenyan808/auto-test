...
# @Author : 
    ...
    步骤/文本
        调用接口：/linear-swap-api/v1/swap_openorders
    预期结果
        A)接口返回的json格式、字段名、字段值正确
    优先级
        0
    用例别名
        TestLinearApiSchema_051
"""
import time
...
@pytest.mark.stable
class TestLinearApiSchema_051:

    @allure.step('前置条件')
    def setup(self):
        self.price = atp.ATP.get_adjust_price(rate=0.98)

    @allure.title('获取用户的合约当前未成交委托（逐仓）')
    @allure.step('测试执行')
    def test_execute(self, contract_code, symbol):
        with allure.step('调用接口：/linear-swap-api/v1/swap_openorders'):
            linear_api.linear_order(contract_code=contract_code,
                                    client_order_id='',
                                    price=self.price,
                                    volume='1',
                                    direction='buy',
