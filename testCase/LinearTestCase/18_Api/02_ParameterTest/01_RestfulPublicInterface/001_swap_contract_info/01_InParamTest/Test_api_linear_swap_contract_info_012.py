...
# @Author : 
    用例标题
        仅不填contract_type参数测试
    前置条件
        
    步骤/文本
        1、调用linear-swap-api/v1/swap_contract_info接口，仅不填contract_type参数。检查返回值有结果A
    预期结果
        A)
    优先级
        1
    用例别名
        Test_api_linear_swap_contract_info_012
"""
...
@pytest.mark.stable
class Test_api_linear_swap_contract_info_012:

    @allure.step('前置条件')
    def setup(self):
        print('''  ''')

    @allure.title('仅不填contract_type参数测试')
    @allure.step('测试执行')
