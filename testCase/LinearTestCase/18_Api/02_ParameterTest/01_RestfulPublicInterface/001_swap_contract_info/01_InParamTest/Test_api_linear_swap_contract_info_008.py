...
class Test_api_linear_swap_contract_info_008:
    ...
    def test_execute(self, contract_code, url):
        with allure.step('1、调用linear-swap-api/v1/swap_contract_info接口，pair参数为空,null。检查返回值有结果A'):
            params = ['', 'null']
            url = url + '/linear-swap-api/v1/swap_contract_info'
            trade_partition = linear_api.get_trade_partition(contract_code)
            for param in params:
                request = {'contract_code': contract_code, 'support_margin_mode': '', 'pair': param,
