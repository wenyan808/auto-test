...
# @Author : 
    ...
    步骤/文本
        1、调用linear-swap-api/v1/swap_open_interest接口，business_type参数在取值范围外。检查返回值有结果A
    预期结果
        A)提示参数错误
    优先级
        1
    用例别名
        Test_api_linear_swap_open_interest_005
"""
...
@pytest.mark.stable
class Test_api_linear_swap_open_interest_005:
    ...
    def test_execute(self):
        with allure.step('1、调用linear-swap-api/v1/swap_open_interest接口，business_type参数在取值范围外。检查返回值有结果A'):
