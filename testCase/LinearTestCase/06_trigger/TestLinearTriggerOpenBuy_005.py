...
class TestLinearTriggerOpenBuy_005:
    ...
    def test_execute(self):
        ...
        offset = 'open'
        leverRate = 5
        with allure.step('1、行情-最新价更新为50500'):
            linear_api.linear_order(contract_code=contractCode,price='50500',order_price_type='limit',lever_rate=5,direction='buy',offset=offset,
                                    volume=1,)
            time.sleep(0.3)
            linear_api.linear_order(contract_code=contractCode, price='50500',order_price_type='limit', lever_rate=5,direction='sell', offset=offset,
                                    volume=1, )
            # 等待成交刷新最新价
            time.sleep(0.3)
            print("》》》》》》》》》》》》》》1、最新价更新为50000")
            pass
        with allure.step('2、计划委托下单，触发价为50000；买入价为45000；'):
        ...
        with allure.step('4、下单使触发后的限价单成交'):
            linear_api.linear_order(contract_code=contractCode, price='45000', order_price_type='limit', lever_rate=5,
                                    direction='sell', offset=offset,
                                    volume=10)
            pass
    @allure.step('恢复环境')
    def teardown(self):
        ...
        linear_api.linear_order(contract_code='BTC-USDT', price='50000', order_price_type='limit', lever_rate=5,
