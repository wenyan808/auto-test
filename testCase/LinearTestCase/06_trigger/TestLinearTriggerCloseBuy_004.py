...
@allure.story('触发价高于最新价-刷新最新价-触发')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
...
class TestLinearTriggerCloseBuy_004:
    ...
    def setup(self,symbol,contract_code,lever_rate,directionB,directionS,offsetC,offsetO):
        print('\n测试标题：触发价高于最新价场景，验证计划委托平空单-委托触发'
              '\n*、先清盘避免盘口数据干扰;'
              '\n*、用户先开仓持仓;'
              '\n*、以触发价高于最新价，买入价低于最新价下平仓计划委托单；'
              '\n*、触发计划委托订单；'
              '\n*、验证计划委托订单触发否')
    ...
    def test_execute(self):
        ...
        with allure.step('6、点击买入平空按钮，弹框点击确认'):
            self.trigger_price = self.highPrice
        ...
        with allure.step('8、验证计划委托单被触发'):
            triggerOrderHistoryOrders = linear_order.linear_swap_his_triggerorders(contract_code=self.contract_code,trade_type=3)
    ...
    def teardown(self):
        print('\n平仓，清理测试数据')
        linear_api.linear_order(contract_code=self.contract_code, price=self.highPrice, order_price_type='limit', lever_rate=self.lever_rate,
                                direction=self.directionB, offset=self.offsetC,
                                volume=1)
        linear_api.linear_order(contract_code=self.contract_code, price=self.lowPrice, order_price_type='limit', lever_rate=self.lever_rate,
