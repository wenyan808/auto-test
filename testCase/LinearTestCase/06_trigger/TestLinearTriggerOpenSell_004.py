...
@pytest.mark.stable
class TestLinearTriggerOpenSell_004:
    ...
    def test_execute(self):
        ...
        with allure.step('2、计划委托下单，触发价为'+triggerPrice+'；开空卖出价为'+orderPrice+'；'):
            ...
            print(orderResult)
            print("》》》》》》》》》》》》》》2、计划委托单下单成功，触发价为"+triggerPrice)
            time.sleep(1)
            # 下单成功后，验证计划委托单成功
            orderInfo = linear_order.linear_swap_open_triggerorders(contract_code=contractCode, trade_type=2)
            print(orderInfo)
        ...
        with allure.step('3、行情-最新价更新；使最新价达到'+triggerPrice+'价，触发计划委托单转换为限制单'):
            ...
            linear_api.linear_order(contract_code=contractCode, price=triggerPrice, order_price_type='limit', lever_rate=5,
                                    ...
                                    volume=1, )
            print("》》》》》》》》》》》》》》3、行情-最新价更新；使最新价达到"+triggerPrice+"价，触发计划委托单转换为限制单")
        ...
        with allure.step('4、下单使触发后的限价单成交'):
            linear_api.linear_order(contract_code=contractCode, price=orderPrice, order_price_type='limit', lever_rate=5,
                                    direction='buy', offset=offset,
                                    volume=10)
            pass
    @allure.step('恢复环境')
