...
@allure.story('撮合 最优20档FOK 买入 平仓')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : Alex Li')
@pytest.mark.stable
class TestLinearEx_063:
    ...
    @allure.title('撮合 最优20档FOK 买入 平仓')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('1、 撮合 最优20档FOK 卖出 开仓'):
            pass
        with allure.step('2、点击“确定按钮”'):
            ...
            # 撮合成交
            ATP.common_user_make_order(
                price=current, direction='sell', offset=offset1)
            time.sleep(1)
