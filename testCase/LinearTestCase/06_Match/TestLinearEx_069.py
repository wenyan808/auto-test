...
@allure.story('撮合 买入开仓挂单 撤单')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : Alex Li')
@pytest.mark.stable
class TestLinearEx_069:
    ...
    @allure.title('撮合 买入开仓挂单 撤单')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('1、 撮合 买入开仓挂单 撤单'):
            pass
        with allure.step('2、点击“确定按钮”'):
            ...
            res = ATP.current_user_make_order(order_price_type=order_price_type, contract_code=contract_code,
                                              price=round(current/2, 2), volume=10, direction=direction, offset=offset)
            pprint(res)
