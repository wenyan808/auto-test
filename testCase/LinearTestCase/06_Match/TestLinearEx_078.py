...
@allure.story('撮合 卖出  开仓全部成交 撤单   ')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : Alex Li')
@pytest.mark.stable
class TestLinearEx_078:
    ...
    @allure.title('撮合 卖出  开仓全部成交 撤单   ')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('1、 撮合 卖出  开仓全部成交 撤单   '):
            pass
        with allure.step('2、点击“确定按钮”'):
            ...
            ATP.common_user_make_order(order_price_type=order_price_type, contract_code=contract_code,
                                       price=current, volume=20, direction='buy', offset='open')
            # 自己卖10,确保能卖出
