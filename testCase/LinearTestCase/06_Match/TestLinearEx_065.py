...
@allure.story('撮合 买入开仓 爆仓 ')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : Alex Li')
@pytest.mark.stable
class TestLinearEx_065:
    ...
    @allure.title('撮合 对手价FOK 卖出 平仓')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('1、撮合 买入开仓 爆仓'):
            pass
        with allure.step('2、点击“确定按钮”'):
            ...
            res_position = linear_api.linear_account_position_info(
                contract_code=contract_code)
            pprint(res_position)
            # 爆仓，用另一个用户反向砸单，拉低价格
            ATP.common_user_make_order(
                contract_code=contract_code, price=current-1, volume=1000, direction='sell', offset=offset)
            time.sleep(1)
