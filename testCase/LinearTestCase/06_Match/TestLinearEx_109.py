...
@allure.story('撮合 买入开仓 部分成交单人多笔价格不同的订单')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : Alex Li')
@pytest.mark.stable
class TestLinearEx_109:
    ...
    @allure.title('撮合 买入开仓 部分成交单人多笔价格不同的订单')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('1、 撮合 买入开仓 部分成交单人多笔价格不同的订单'):
            pass
