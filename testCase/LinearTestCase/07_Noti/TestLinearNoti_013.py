...
步骤/文本
    请求聚合行情(单个合约，即传参contract_code),可参考文档：https://docs.huobigroup.com/docs/usdt_swap/v1/cn/#0737c93bf7
预期结果
...
class TestLinearNoti_013:
    ...
    def test_execute(self, contract_code):
        with allure.step('请求聚合行情(单个合约，即传参contract_code),可参考文档：https://docs.huobigroup.com/docs/dm/v1/cn/#websocket-3'):
            # open、close、low、high价格正确；amount、vol、count值正确asks、bid值正确,不存在Null,[]
            res = api.linear_detail_merged(contract_code=contract_code)
            pprint(res)
            ...
            check_keys = ['vol', 'count', 'open', 'close', 'low', 'high', 'amount', 'trade_turnover', 'ask', 'bid',
                          'id',
                          'ts']
