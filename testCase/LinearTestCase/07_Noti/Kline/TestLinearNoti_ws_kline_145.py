...
@allure.story('WS订阅K线(req) 1mon 合约代码小写')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestLinearNoti_ws_kline_145:

    @allure.step('前置条件')
    def setup(self):
        print("\n自动化步骤："
              "\n*、发送req请求from to请求kline ，请求参数，合约代码小写；"
              "\n*、验证Kline 1mon返回结果；大小写不敏感，正常返回")

    @allure.title('WS订阅K线(req) 合约代码小写')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            self.contract_code = contract_code.lower()
            self.period = '1min'
            self.toTime = int(time.time())
            self.fromTime = self.toTime - 60 * 60 * 3
            subs = {
