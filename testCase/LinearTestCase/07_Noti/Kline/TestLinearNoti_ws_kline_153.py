...
@allure.story('WS订阅K线(req) 4hour数据超过2000条')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestLinearNoti_ws_kline_153:

    @allure.step('前置条件')
    def setup(self):
        print("\n自动化步骤："
              "\n*、发送req请求from to请求kline ，请求参数，4hour数据超过2000条；"
              "\n*、验证Kline 1mon返回结果；time gap too large")

    @allure.title('WS订阅K线(req) 4hour数据超过2000条')
    @allure.step('测试执行')
