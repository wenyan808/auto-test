...
@allure.story('WS订阅K线(req) period不存在 合约正确')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestLinearNoti_ws_kline_139:

    @allure.step('前置条件')
    def setup(self):
        print("\n自动化步骤："
              "\n*、发送req请求from to请求kline ，请求参数，period不存在 合约正确；"
              "\n*、验证Kline 1mon返回结果；invalid kline type")

    @allure.title('WS订阅K线(req) period不存在 合约正确')
    @allure.step('测试执行')
