...
@allure.story('WS订阅K线(req 传参from,to) 1week')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestLinearNoti_ws_kline_017:

    @allure.step('前置条件')
    def setup(self):
        print("\n自动化步骤："
              "\n*、发送req请求kline 1week的请求；"
              "\n*、验证Kline 1week返回结果；各返回值不为空[]")

    @allure.title('WS订阅K线(req 传参from,to) 1week')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            ...
            self.fromTime = self.toTime - 60 * 60 * 24 * 30 * 6
            subs = {
