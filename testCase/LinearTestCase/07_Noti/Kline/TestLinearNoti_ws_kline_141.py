...
@allure.story('WS订阅K线(req) 1mon 不传period')  # 这里填子功能，没有的话就把本行注释掉
@pytest.mark.stable
@allure.tag('Script owner : 余辉青', 'Case owner : 吉龙')
class TestLinearNoti_ws_kline_141:

    @allure.step('前置条件')
    def setup(self):
        print("\n自动化步骤："
              "\n*、发送req请求from to请求kline ，请求参数，不传period；"
              "\n*、验证Kline 1mon返回结果；invalid topic")

    @allure.title('WS订阅K线(req) 不传period')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            ...
            subs = {
                "req": "market.{}.kline".format(self.contract_code),
                "id": "id4",
                "from": self.fromTime,
                "to": self.toTime
            }
