...
# @Author : chenwei
    ...
    优先级
        0
    用例别名
        TestLinearNoti_restful_kline_073
"""
...
@pytest.mark.stable
class TestLinearNoti_restful_kline_073:
    ...
    @allure.title('restful请求K线 1mon 传参from,to')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            r = linear_api.linear_kline(contract_code=contract_code, period="1mon", FROM=self.from_time,
                                        to=self.to_time)
            pprint(r)
            klinedata = r['data'][0]
