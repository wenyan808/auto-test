...
# @Author : chenwei
    用例标题
        restful请求K线 1mon from传参错误 非时间戳
    前置条件
    ...
    优先级
        3
    用例别名
        TestLinearNoti_restful_kline_078
"""
...
@pytest.mark.stable
class TestLinearNoti_restful_kline_078:
    ...
    @allure.title('restful请求K线 1mon from传参错误 非时间戳')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            r = linear_api.linear_kline(contract_code=contract_code, period="1mon", size=2, FROM="10000", to=self.to_time)
            pprint(r)
            klinedata = r['data'][0]
