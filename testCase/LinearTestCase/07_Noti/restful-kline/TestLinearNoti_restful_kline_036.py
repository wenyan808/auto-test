...
# @Author : chenwei
    用例标题
        restful请求K线 30min 不传from,to,size
    前置条件
    ...
    优先级
        3
    用例别名
        TestLinearNoti_restful_kline_036
"""
...
@pytest.mark.stable
class TestLinearNoti_restful_kline_036:
    ...
    @allure.title('restful请求K线 60min 不传from,to,size')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            r = linear_api.linear_kline(contract_code=contract_code, period="60min", size=2, FROM=self.from_time,
                                        to=self.to_time)
            pprint(r)
            klinedata = r['data'][0]
