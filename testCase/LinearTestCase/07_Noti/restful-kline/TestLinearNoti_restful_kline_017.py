...
# @Author : chenwei
    ...
    优先级
        3
    用例别名
        TestLinearNoti_restful_kline_017
"""
...
@pytest.mark.stable
class TestLinearNoti_restful_kline_017:
    ...
    @allure.title('restful请求K线 5min size>2000')
    @allure.step('测试执行')
    def test_execute(self, contract_code):
        with allure.step('详见官方文档'):
            r = linear_api.linear_kline(contract_code=contract_code, period="5min", FROM=self.from_time, to="10000",
                                        size=2010)
            pprint(r)
            errmsg = r['err-msg']
