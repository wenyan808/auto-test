...
步骤/文本
    请求批量成交记录(单个合约，即传参contract_code),可参考文档：https://docs.huobigroup.com/docs/usdt_swap/v1/cn/#0737c93bf7
预期结果
...
class TestLinearNoti_017:
    ...
    def test_execute(self, contract_code):
        with allure.step(
                '请求批量成交记录(单个合约，即传参contract_code),可参考文档：https://docs.huobigroup.com/docs/usdt_swap/v1/cn/#0737c93bf7'):
            # amount\quantity\turnover\direction\price显示正确,不存在Null,[]
            res = api.linear_history_trade(contract_code, 2)
            print()
            pprint(res)
            ...
            for tick in data:
                ...
                trade_record = data[0]
                check_keys = ['amount', 'trade_turnover', 'direction', 'id', 'price', 'quantity', 'ts']
                ...
                trade_turnover = trade_record.get('trade_turnover', 0)
