...
@allure.story('子账户逐仓划转到母账户逐仓（挂多单）')  # 这里填子功能，没有的话就把本行注释掉
@allure.tag('Script owner : Alex Li', 'Case owner : Alex Li')
@pytest.mark.stable
class TestUSDTSwapTransfer_075:

    @allure.step('前置条件')
    def setup(self):
        print("前置条件")

    @allure.title('子账户逐仓划转到母账户逐仓（挂多单）')
    @allure.step('测试执行')
    def test_execute(self, sub_uid, symbol, contract_code):
        ...
        with allure.step('6、点击“确定按钮”'):
            ...
            res = ATP.current_user_make_order(
                contract_code=contract_code, price=current, volume=10, direction=direction, offset=offset, iscross=False)
            pprint(res)
            ...
            res = linear_api.linear_master_sub_transfer(from_margin_account=from_margin_account, to_margin_account=to_margin_account,
                                                        amount=amount,
                                                        sub_uid=sub_uid,
